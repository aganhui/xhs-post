---
name: xhs-post
description: 小红书笔记发布主流程。本地生成文案+图片，扫码发布到小红书。触发词：发小红书、xhs、小红书笔记、发布笔记。
version: 1.0.0
---
...
Step 1: 本地生成文案（免费）
  → 调用 xhs-copy 规则，用当前 LLM 生成

Step 2: 本地生成图片（免费）
  → 调用 xhs-html-card，生成 HTML 卡片并截图
...
| `xhs-publish` | 云服务 API 完整使用说明 | 按用量付费 |

详细 API 参考见 `xhs-publish` skill 及其 `references/` 目录。

## Agent 交互规范

1. **本地优先**：文案和图片默认本地生成（免费），用户要求时才用服务器 API
2. **预览优先**：创建笔记后，**必须**将 `preview_url` 展示给用户
3. **确认发布**：发布前**必须**询问用户，收到明确肯定后才调用 publish
4. **图片必须**：没有图片不允许发布
5. **余额检查**：402 错误 → 告知余额不足
6. **标签在正文里**：标签以 `#标签` 格式写在正文末尾，不需要单独字段
7. **二维码展示**：publish 返回 qrcode 后，**必须**展示给用户让其扫码
